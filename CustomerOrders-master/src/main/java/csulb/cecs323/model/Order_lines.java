...
@NamedNativeQueries({
        @NamedNativeQuery(
                name="ReturnTotal",
                query = "SELECT customer_id, SUM(quantity * unit_sale_price) " +
                        "FROM   ORDER_LINES " +
                        "WHERE customer_id = ? " +
...
public class Order_lines {
    ...
    public Order_lines(int quantity, double price, Orders order, Customers customerID, Products upc){
        this.quantity = quantity;
        this.unit_sale_price = price;
        //this.order.setOrder_date(order.getOrder_date());
        this.setOrder(order);
        this.order.setCustomer(customerID);
